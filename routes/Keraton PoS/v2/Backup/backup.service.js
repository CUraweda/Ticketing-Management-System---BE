...
const getAllTabel = async () => {
  ...
  while ((match = modelRegex.exec(schema)) !== null) {
    ...
    const relationCount = [
      ...modelBody.matchAll(/\w+\s+(\w+)\s+@relation\([^)]*\)/g),
    ].map((match) => match[1]);
    const arrayFieldTypes = [...modelBody.matchAll(/\w+\s+(\w+)\[\]/g)].map(
      (match) => match[1]
    );
    const uniqueFields = [...modelBody.matchAll(/(\w+)\s+\w+\s+@unique/g)].map(
      (match) => match[1]
    );
  ...
  try {
        ...
        for (let dataIndex in tableData) {
          ...
          if (data.depended.length < 1) {
            await prisma[data.name].deleteMany().catch((err) => {
              console.log(`${client.dbName} gagal dihapus, Error occured`);
            });
            console.log(
          ...
          for (let dependedName of data.depended) {
            const dependedReference = tableData.find(
              (model) => model.name === dependedName
            );
    ...
    while (dataReferences.length > 0) {
      for (let clientIndex in dataReferences) {
        ...
        const dataToDb = backupDatas
          .filter((dataBackup) => dataBackup[uniqueFields])
          .map((dataBackup) => {
            if (deleteDatabase != "true") delete dataBackup.id;
            return dataBackup;
          });
        ...
        const dependedData = client.load
          .map((loadName) =>
            dataReferences.find(
              (model) => model.dbName === changeToDBName(loadName)
            )
          )
          .filter((dependedReference) => dependedReference);
        if (dependedData.length < 1) {
          await backupClientData(
            client,
            dataToDb,
