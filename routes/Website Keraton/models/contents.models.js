...
const createUpdate = async (ident, id, data) => {
  ...
  try {
    if (ident != "create")
      await isExist(id).then((exist) => {
        if (!exist) throw Error("Content ID didnt Exist");
      });
    if (textList) {
      for (let textIndex = 0; textIndex < textList.length; textIndex++)
        context[`xs${textIndex + 1}`] = textList[textIndex];
    ...
    if (imageList) {
      for (let imgIndex = 0; imgIndex < imageList.length; imgIndex++) {
        ...
        imageData.data = imageData.data?.path
          ? convertFilesToURL(imageData.data.path)
          : imageData.data;
        context[`xi${imgIndex + 1}`] = imageData;
        delete data.imageList;
        delete data.imageSub;
      }
    }
    let dataToReturn;
    if (ident != "create") {
      dataToReturn = await prisma.contents.update({
        where: { id },
        data: { ...data, context },
      });
